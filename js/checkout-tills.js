...
function checkoutTills(customer, n){
    // create an array of tills
    const tills = new Array(n).fill(0)

    // loop thru the length of the customers
    for(let i = 0; i < customer.length; i++){
        // next customer becomes each customer
        let nextCustomer = customer[i]
        // next till is the till with the minimum time
        let nextTill = tills.indexOf(Math.min(...tills))
        // Add the time of the next customer to the selected till
        tills[nextTill] += nextCustomer
    }
    return Math.max(...tills) // Return the maximum time among all tills
}

console.log(checkoutTills([10,2,3,3], 2), 10)