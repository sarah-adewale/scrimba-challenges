...
const removeDuplicates = (nums) => {
    // if the length of the array is 2, return the length
    if(nums.length === 2){
        return nums.length
    }
    let uniqueNum = 1
    let duplicateNum = 1
    // create a loop to check for unique numbers
    for(let i = 1; i < nums.length; i++){
        if(nums[i] !== nums[i - 1]){
            ...
            // if the duplicate numbers are less than 2
            if(duplicateNum < 2){
                nums[uniqueNum] = nums[i]
                uniqueNum++
            }
    ...
    return uniqueNum
    // check if the unique characters have ocuured more than twice
}
